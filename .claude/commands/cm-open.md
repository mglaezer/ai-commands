...
7. If worktree exists but no workspace: `cmux new-workspace --cwd <worktree-path> --command "cd <worktree-path>/<relative-subdir> 2>/dev/null; claude --dangerously-skip-permissions"` (returns the new workspace ref), `cmux rename-workspace --workspace <new-ref> <branch>`, switch to it.
8. If no worktree: create it (for remote-only branches use `git worktree add -b <branch> <path> origin/<branch>`), `cmux new-workspace --cwd <worktree-path> --command "cd <worktree-path>/<relative-subdir> 2>/dev/null; claude --dangerously-skip-permissions"` (returns the new workspace ref), `cmux rename-workspace --workspace <new-ref> <branch>`, switch to it.
